...
import { MOCK_WSJF_ITEMS, MOCK_RICE_ITEMS } from '@/lib/mock-data'
...
function WSJFRow({
  ...
  item: WSJFItem
  isWinner: boolean
  onChange: (updated: WSJFItem) => void
  onDelete: () => void
}) {
      ...
      {(['userBusinessValue', 'timeCriticality', 'riskReductionOpportunityEnablement', 'jobDuration'] as const).map(field => (
        <div key={field} className="flex justify-center">
          <input
            ...
            className="glass-input w-16 py-1.5 rounded-lg text-center text-on-surface font-bold text-sm"
            min={0}
          />
        </div>
      ))}
...
function RICERow({
  ...
  item: RICEItem
  isWinner: boolean
  onChange: (updated: RICEItem) => void
  onDelete: () => void
}) {
  ...
  return (
    <div className={`grid grid-cols-[1fr_100px_80px_100px_80px_120px_70px_36px] gap-3 px-5 py-4 rounded-2xl items-center transition-all group relative overflow-hidden ${
      isWinner
        ? 'bg-secondary/10 border border-secondary/20 hover:bg-secondary/15'
        : 'bg-white/3 border border-white/5 hover:bg-white/6'
    }`}>
      ...
      {(['reach', 'impact', 'confidence', 'effort'] as const).map(field => (
        <div key={field} className="flex justify-center">
          <input
            ...
            step={field === 'impact' ? 0.25 : undefined}
          />
        </div>
      ))}
...
function PrioritizationContent() {
  ...
  const [activeTab, setActiveTab] = useState<'wsjf' | 'rice'>(
    (searchParams.get('tab') as 'wsjf' | 'rice') ?? 'wsjf'
  )
  const [wsjfItems, setWsjfItems] = useState<WSJFItem[]>(rankWSJFItems(MOCK_WSJF_ITEMS))
  const [riceItems, setRiceItems] = useState<RICEItem[]>(rankRICEItems(MOCK_RICE_ITEMS))
  const [addOpen, setAddOpen] = useState(false)

  // Re-rank on each change
  useEffect(() => {
    setWsjfItems(prev => rankWSJFItems(prev))
  }, [])
  ...
  const addFeature = (name: string) => {
    if (activeTab === 'wsjf') {
      const newItem: WSJFItem = { id: `w${Date.now()}`, name, userBusinessValue: 5, timeCriticality: 5, riskReductionOpportunityEnablement: 3, jobDuration: 3, wsjfScore: 0 }
      setWsjfItems(prev => rankWSJFItems([...prev, newItem]))
    } else {
      const newItem: RICEItem = { id: `r${Date.now()}`, name, reach: 1000, impact: 2, confidence: 80, effort: 5, riceScore: 0 }
      setRiceItems(prev => rankRICEItems([...prev, newItem]))
    }
  }
    ...
    <div className="p-6 lg:p-8 space-y-6 max-w-[1600px]">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 animate-fade-in">
        ...
        <div className="glass-card p-1.5 rounded-2xl flex relative min-w-[280px] border-white/8">
          <div
            className={`absolute top-1.5 bottom-1.5 w-[calc(50%-3px)] bg-white/10 border border-white/10 rounded-xl transition-transform duration-500 z-0 ${
              activeTab === 'rice' ? 'translate-x-[calc(100%+6px)]' : 'translate-x-0'
            }`}
          />
          <button
            onClick={() => setActiveTab('wsjf')}
            className={`relative z-10 flex-1 py-2.5 rounded-xl text-label-caps text-[12px] tracking-widest transition-all ${
              ...
              <div className="space-y-3">
                {wsjfItems.map(item => (
                  <WSJFRow
                    ...
                    onChange={updateWSJF}
                    onDelete={() => setWsjfItems(prev => rankWSJFItems(prev.filter(i => i.id !== item.id)))}
                  />
                ))}
              </div>
            </div>
              ...
              <div className="space-y-3">
                {riceItems.map(item => (
                  <RICERow
                    ...
                    isWinner={item.rank === 1}
                    onChange={updateRICE}
                    onDelete={() => setRiceItems(prev => rankRICEItems(prev.filter(i => i.id !== item.id)))}
                  />
                ))}
              </div>
            </div>
